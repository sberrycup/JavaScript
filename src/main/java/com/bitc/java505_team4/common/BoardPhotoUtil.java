...
@Component
public class BoardPhotoUtil {
    public List<BoardDTO> parseFileInfo(BoardDTO board, MultipartHttpServletRequest uploadFiles) throws Exception {

        if (ObjectUtils.isEmpty(uploadFiles)) {
            return null;
        }

        List<BoardDTO> fileList = new ArrayList<BoardDTO>();

//    DateTimeFormatter format = DateTimeFormatter.ofPattern("yyyy-MM-dd");
//    ZonedDateTime current = ZonedDateTime.now();

//    String path = "C:/upload/" + current.format(format);
        String savedPath = "/BoardPhoto";
        // 절대경로로 구현, 변경필요
        String realPath = "C:\\Users\\kaon7\\Desktop\\스프링 팀프로젝트\\java505_team4(2023-07-10 합치기 전)\\java505_team4(회원관련,7월5일기준보드)\\src\\main\\resources\\static\\boardPhoto";
        ...
        while (iterator.hasNext()) {
            ...
            for (MultipartFile multipartFile : fileLists) {
                if (multipartFile.isEmpty() == false) {
                    ...
                    board.setBoardImg(multipartFile.getOriginalFilename()); //파일 원본 이름
                    board.setBoardFile(savedPath + "/" + newFileName); //저장명
